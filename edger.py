...
for fn in os.listdir(root):
    ...
    if os.path.isdir(file) and 'U' in file:
        cut = open(os.path.join(file, 'cut.json'), 'r').read()
        cut = json.loads(cut)
...
classifier = tf.estimator.Estimator(model_fn=model, model_dir="/tmp/net_edger")

# Set up logging for predictions
tensors_to_log = {"line": "line"}
...
for single_prediction in predict:
    image = images[i]
    print(np.round(single_prediction['line']))
    im = Image.open(image)

    draw = ImageDraw.ImageDraw(im)
    draw.line(np.round(single_prediction['line']), 128, 6)
